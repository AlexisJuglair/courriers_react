...
export default function AppCourrierFormulaire(props) 
{
  ...
  useEffect(() => 
  {
    ...
    {
      fetch(`http://courriers.server1/api/courrier/${user}/${sessionStorage.getItem('id')}`)
      .then(response => 
      {
        return response.json();
      })
  ...
  const onSubmit = (e) =>
  {
    ...
    if (props.action == "ajouter")
    {
      ...
      {
        fetch(`http://courriers.server1/api/courrier/ajouter/${user}`, 
        {
          method: "POST",
          body: formData
        })
    ...
    else
    {
      ...
      {
        fetch(`http://courriers.server1/api/courrier/modifier/${user}/${records[0][0]}`, 
        {
          method: "PUT",
