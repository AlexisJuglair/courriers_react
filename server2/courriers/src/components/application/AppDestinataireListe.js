...
export default function AppDestinataireListe(props) 
{
  ...
  useEffect(() => 
  {
    fetch(`http://courriers.server1/api/destinataires/liste/${user}`)
    .then(response => 
    {
    ...
    .then(r =>
    {
      ...
      setMainTitle("Liste des destinataires");
    })
  }, [])
  ...
  const onSupprimer = (e) =>
  {
    ...
    fetch(`http://courriers.server1/api/destinataire/supprimer/${user}/${id}`,
    {
      method: "DELETE",
    })
    .then(response => 
          ...
          <Modal h="200px" w="400px">
            ...
            <ModalBody>
              <p>
                ...
                Le destinataire a été supprimé !
              </p>
            </ModalBody>
            <ModalFoot>
              <Menu float="right" >
  ...
  const onSupprimerMenu = () =>
  {
    ...
    if (Object.keys(lines).length > 0)
    {
      ...
      fetch(`http://courriers.server1/api/destinataires/supprimer/${user}/${tables_id}`,
      {
        method: "DELETE",
      })
      .then(response => 
          ...
          return (
            <Modal h="200px" w="400px">
              ...
              <ModalBody>
                <p>
                  <i className="fas fa-check-circle fa-lg" style={{color: "#4caf50", marginRight: "5px"}}></i>
                  Les destinataires ont été supprimés !
                </p>
              </ModalBody>
              <ModalFoot>
                <Menu float="right" >
          ...
          <Modal h="200px" w="400px">
            ...
            <ModalBody>
              ...
              <i className="fas fa-times-circle fa-lg" style={{color: "#b71c1c", marginRight: "5px"}}></i>
                Aucun destinataire à supprimer !
              </p>
            </ModalBody>
            <ModalFoot>
              <Menu float="right" >
      ...
      <Table>
        <Theadbody element="thead" >
          <Tr>
            ...
            <Thd element="th" align="left" label="Nom" />
            <Thd noneResponsive={1} innerWidth={768} element="th" align="left" label="Fonction" />
            <Thd element="th" align="left" label="Dénomination" />
            <Thd noneResponsive={1} innerWidth={768} element="th" align="left" label="Localité" />
        ...
        {records[0] !== undefined &&
          <Theadbody element="tbody" >
            {records.map((record, r) =>
              <Tr key={r} table="destinataires" recordId={record[0]} click={clickLine} >
                <Thd noneResponsive={1} innerWidth={768} element="td" align="right" label={record[0]} click={clickLine} />
                <Thd element="td" align="left" label={`${record[4]} ${record[3]}`} click={clickLine} />
                <Thd noneResponsive={1} innerWidth={768} element="td" align="left" label={record[5]} click={clickLine} />
                <Thd element="td" align="left" label={record[2]} click={clickLine} />
                <Thd noneResponsive={1} innerWidth={768} element="td" align="left" label={`${record[6]} ${record[7]}`} click={clickLine} />
                <Thd noneResponsive={1} innerWidth={768} element="td" align="center">
                  <IconeAction table="destinataires" action="modifier" recordId={record[0]} class="fas fa-edit" click={onModifier} />
                  <IconeAction table="destinataires" action="supprimer" recordId={record[0]} class="fas fa-trash-alt" click={onSupprimer}/>
                </Thd>
              </Tr>
            )}
          </Theadbody>
