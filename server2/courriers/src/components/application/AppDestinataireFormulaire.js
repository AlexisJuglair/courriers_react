...
export default function AppDestinataireFormulaire(props) 
{
  ...
  useEffect(() => 
  {
    ...
    {
      fetch(`http://courriers.server1/api/destinataire/${user}/${sessionStorage.getItem('id')}`)
      .then(response => 
      {
  ...
  const onSubmit = (e) =>
  {
    ...
    if (props.action === "ajouter")
    {
      ...
      {
        fetch(`http://courriers.server1/api/destinataire/ajouter/${user}`, 
        {
          method: "POST",
          body: formData
        })
    ...
    else if (props.action === "modifier")
    {
      ...
      else
      {
        ...
        fetch(`http://courriers.server1/api/destinataire/modifier/${user}/${records[0][0]}`, 
        {
          method: "PUT",
