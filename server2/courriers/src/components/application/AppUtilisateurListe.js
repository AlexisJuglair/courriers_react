...
export default function AppUtilisateurListe(props) 
{
  ...
  useEffect(() => 
  {
    fetch(`http://courriers.server1/api/utilisateurs/liste/${user}`)
    .then(response => 
    {
  ...
  const onSupprimer = (e) =>
  {
    ...
    fetch(`http://courriers.server1/api/utilisateur/supprimer/${user}/${id}`,
    {
      method: "DELETE",
    })
    .then(response => 
  ...
  const onSupprimerMenu = () =>
  {
    ...
    if (Object.keys(lines).length > 0)
    {
      ...
      fetch(`http://courriers.server1/api/utilisateurs/supprimer/${user}/${tables_id}`,
      {
        method: "DELETE",
      })
      .then(response => 
