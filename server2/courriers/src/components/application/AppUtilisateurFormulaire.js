...
export default function AppUtilisateurFormulaire(props) 
{
  ...
  useEffect(() => 
  {
    if (props.action === "modifier" && role === "admin")
    {
      id = id === null ? user : id;
      console.log(id);
      fetch(`http://courriers.server1/api/utilisateur/${user}/${id}`)
      .then(response => 
      {
    ...
    {
      fetch(`http://courriers.server1/api/utilisateur/${user}`)
      .then(response => 
      {
        return response.json();
      })
  ...
  const onSubmit = (e) =>
  {
    ...
    if (props.action === "inscrire")
    {
      ...
      {
        console.log(document.getElementById('mot_de_passe').value+" "+document.getElementById('confirmation_mot_de_passe').value);
        fetch(`http://courriers.server1/api/utilisateur/ajouter`, 
        {
          method: "POST",
          body: formData
        })
    ...
    else if (props.action === "modifier")
    {
      ...
      else if (document.getElementById('nouveau_mot_de_passe').value === document.getElementById('confirmation_mot_de_passe').value) 
      {
        ...
        if (role === "admin") 
        {
          console.log(`http://courriers.server1/api/utilisateur/modifier/${user}/${id}`);
          fetch(`http://courriers.server1/api/utilisateur/modifier/${user}/${id}`, 
          {
            method: "PUT",
        ...
        {
          console.log(`http://courriers.server1/api/utilisateur/modifier/${user}`);
          fetch(`http://courriers.server1/api/utilisateur/modifier/${user}`, 
          {
            method: "PUT",
            body: serialize_form(this),
            headers: {'content-type':'application/json'}
