...
export default function AppCourrierListe(props) 
{
  ...
  useEffect(() => 
  {
    ...
    fetch(`http://courriers.server1/api/courriers/liste/${user}`)
    .then(response => 
    {
  ...
  const onSupprimer = (e) =>
  {
    ...
    fetch(`http://courriers.server1/api/courrier/supprimer/${user}/${id}`,
    {
      method: "DELETE",
    })
    .then(response => 
  ...
  const onSupprimerMenu = () =>
  {
    ...
    if (Object.keys(lines).length > 0)
    {
      ...
      fetch(`http://courriers.server1/api/courriers/supprimer/${user}/${tables_id}`,
      {
        method: "DELETE",
      })
      .then(response => 
